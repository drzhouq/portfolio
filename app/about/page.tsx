...
export default async function About() {
  ...
  const bio = settings.aboutBio ?? DEFAULT_BIO;
  const skills = settings.aboutSkills ?? DEFAULT_SKILLS;
  const profilePhoto = settings.profilePhotoUrl || "/images/profile.png";
      ...
      <article>
        <div className="flex flex-col md:flex-row gap-8 items-start">
          ...
          <div>
            ...
            {bio.map((paragraph, i) => (
              <p key={i} className="text-dark/80 mb-4 leading-relaxed">
                {paragraph}
              </p>
            ))}

            <h3 className="text-xl font-bold text-dark mb-4 mt-8">
              Skills &amp; Expertise
            </h3>
