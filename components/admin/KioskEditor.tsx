...
export default function KioskEditor({ artworks, settings, onUpdate }: KioskEditorProps) {
  const [config, setConfig] = useState<KioskConfig>(settings.kiosk ?? DEFAULT_KIOSK);
  ...
  const visibleArtworks = artworks.filter((a) => a.visible);
  const selectedArtworks = config.artworkIds
    .map((id) => visibleArtworks.find((a) => a.id === id))
    .filter(Boolean) as Artwork[];
  const availableArtworks = visibleArtworks.filter(
